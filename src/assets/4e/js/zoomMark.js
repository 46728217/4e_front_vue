		...
		this.publicMethods = {
			init:function(options){
              ...
              var  markStyle='width:30px;height:30px;position:absolute;transform:translate(-50%,-50%);background-image: url("/static/4e/pointer.png");background-size: 100%;background-repeat: no-repeat;';
				...
				resetImg();
				var list=JSON.parse(localStorage.getItem('mMarks'));
                if(list){
                    for(var i=0;i<list.length;i++){
                        mContainer.publicMethods.mark(list[i].x,list[i].y,list[i].unid,list[i].tid,list[i].type);
                    }
				}
			},
			mark:function(x,y,unid,tid,type,name,content){
				...
				mContainer.append($('<div unid="'+newMark.unid+'" tid="'+newMark.tid+'"  type="'+newMark.type+'" class="mark" id="mark_'+newMark.id+'" style="background-color:'+data.settings.markColor+'"></divtid>'));
